...
- **系统通知**（主题设置）：调用浏览器 Notification API，在**对话运行结束**、以及**出现选择/问题建议**（ask_user_question）时发送系统级通知；点击通知聚焦窗口直达结果或待回答的问题。开启开关时会请求通知权限。
...
7. **系统通知**
   - 开关存于 `ui-theme.frostedNotifyEnabled`（默认开启）；开启开关时调用 `Notification.requestPermission()` 请求权限（在用户手势内，浏览器允许弹窗）。
...
- 「主题设置 → 系统通知」默认开启：首次开启会请求通知权限；一轮对话结束后收到「对话结束」系统通知，出现选择/问题建议时收到「需要你的选择」通知，点击通知聚焦窗口。
