...
- 莫奈主题默认不显示自定义背景图；开启「应用自定义背景图」后可在莫奈主题下显示全局背景图/视频/幻灯片，并用透明度滑块调节图片透明度（0=不透明、背景清楚，100=完全透明、背景隐藏）。所有主界面表面都会半透明以透出背景壁纸；设置面板及其中的下拉/菜单/对话框等浮层，以及输入框、选择器等交互控件保持不透明；工作区/内容区整体透出壁纸。仍不使用毛玻璃模糊，与毛玻璃主题完全独立。